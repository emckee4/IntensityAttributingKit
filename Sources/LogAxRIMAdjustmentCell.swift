...
final class LogAxRIMAdjustmentCell:RawIntensityAdjustmentCellBase {
    ...
    override init() {
        ...
        thresholdSV.slider.addTarget(self, action: #selector(LogAxRIMAdjustmentCell.updateRIM(_:)), for: .valueChanged)
        ...
        ceilingSV.slider.addTarget(self, action: #selector(LogAxRIMAdjustmentCell.updateRIM(_:)), for: .valueChanged)
        
        aParamSV = LabeledSliderView(labelText: "\"a\" Parameter", sliderMin: 1, sliderMax: 100.0, sliderValue: LogAxMapping.aParam)
        contentStackView.addArrangedSubview(ceilingSV)
        aParamSV.slider.addTarget(self, action: #selector(LogAxRIMAdjustmentCell.updateRIM(_:)), for: .valueChanged)
