...
extension IAString {
    ...
    func unitRanges(withOverridingOptions options:IAStringOptions?)->[CountableRange<Int>]{
    ...
    ///Gets an array of ranges or Character/word/sentance/etc units, as determined by the separation option
    fileprivate func unitRanges(_ tokenizer:IAStringTokenizing)->[CountableRange<Int>]{
        guard self.length > 0 else {return []}
        guard let separationOptions = tokenizer.enumerationOption else {return [0..<((self.text as NSString).length)]  }//assume .Message
        var rangeArray:[CountableRange<Int>] = []
        (self.text as NSString).enumerateSubstrings(in: NSRange(location: 0, length: self.length), options: separationOptions) { (subString, strictSSRange, enclosingRange, stop) -> Void in
            if subString != nil {
                rangeArray.append(enclosingRange.toCountableRange())
            } else {
    ...
    fileprivate func perUnitSmoothedIntensities(_ iaTokenizer:IAStringTokenizing)->CollapsingArray<Int>{
        ...
        for range in self.unitRanges(iaTokenizer) {
            ...
            let avgForRange:Int = self.intensities[range].reduce(0, +) / rangeLength
    ...
    fileprivate func binnedSmoothedIntensities(_ bins:Int, usingTokenizer: IAStringTokenizing)->CollapsingArray<Int>{
    ...
    static func binNumberForSteps(_ intensity:Int, steps:Int)->Int{
    ...
    internal func convertRangeToNSAttributedStringExtendingBoundaries(_ range:CountableRange<Int>, withOverridingOptions options:IAStringOptions? = nil)->(rangeModified:CountableRange<Int>,attString:NSAttributedString) {
        ...
        let startBinDataIndex = smoothedBinned.data.index(where: {$0.range.overlaps(range)})!
        let endBinDataIndex = smoothedBinned.data[smoothedBinned.data.indices.suffix(from: startBinDataIndex)].index(where: {!($0.range.overlaps(range))}) ?? smoothedBinned.data.endIndex
    ...
    public func convertToNSAttributedString(withOverridingOptions options:IAStringOptions? = nil)->NSAttributedString{
        ...
        for attachTupple in attachments {
            assert(self.text.subStringFromRange(attachTupple.loc..<(attachTupple.loc + 1)) == "\u{FFFC}")
    ...
    fileprivate func applyAttributesForStaticDisplay(_ attString:NSMutableAttributedString, transformer:IntensityTransforming.Type,smoothedBinned:CollapsingArray<Int>){
        ...
        while currentIndex < textLength {
            ...
            let binEnd = currentBin.range.upperBound
            let attsEnd = currentAtts.range.upperBound
            var endIndex:Int = 0
            if binEnd < attsEnd {
                endIndex = binEnd
...
//                    currentAtts =  baseAttributes.rvp(++attsDi)
    ...
    func checkRangeIsIndependentInRendering(_ range:CountableRange<Int>, overridingOptions options:IAStringOptions? = nil)->Bool{
        guard range.isEmpty == false else {return true}
        let usingOptions = (self.baseOptions ?? IAKitPreferences.iaStringDefaultBaseOptions).optionsWithOverridesApplied(options)
        let renderedRanges = self.unitRanges(usingOptions.preferedSmoothing)
        if let startRangeIndex = renderedRanges.index(where: {$0.lowerBound == range.lowerBound}) {
            if renderedRanges[startRangeIndex..<renderedRanges.count].contains(where: {$0.upperBound == range.upperBound}) {
                return true
            }
        }
        return false
    ...
    func rangeContainsAttachments(_ range:CountableRange<Int>)->Bool{
...
extension IAString {
    ...
    fileprivate func applyAttributesForLayeredDisplay(topAttString:NSMutableAttributedString, bottomAttString:NSMutableAttributedString,transformer:AnimatedIntensityTransforming.Type,smoothedBinned:CollapsingArray<Int>){
    ...
    ///Renders a substring out to the boundaries of the tokenized text, which may extend further than the requested range.
    internal func convertRangeToLayeredAttStringExtendingBoundaries(_ range:CountableRange<Int>, withOverridingOptions options:IAStringOptions? = nil)->(rangeModified:CountableRange<Int>,topAttString:NSAttributedString, botAttString:NSAttributedString?) {
//        var usingOptions = self.baseOptions ?? IAKitPreferences.iaStringDefaultBaseOptions
//        if options != nil {
//            usingOptions = usingOptions.optionsWithOverridesApplied(options!)
//        }
        ...
        var modRange:CountableRange<Int>!
        if let startBinDataIndex = smoothedBinned.data.index(where: {$0.range.overlaps(range)}) {
            
            let endBinDataIndex = smoothedBinned.data[smoothedBinned.data.indices.suffix(from: startBinDataIndex)]
                .index(where:{ !($0.range.overlaps(range)) } ) ?? smoothedBinned.data.endIndex
